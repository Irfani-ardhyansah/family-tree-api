...
export class MemoriamRepository {
  async countTributesByDeceasedIds(deceasedIds: number[]): Promise<Map<number, number>> {
    ...
    const rows = await db(Tables.MEMORIAM_TRIBUTES)
  ...
  async countPrayersByDeceasedIds(deceasedIds: number[]): Promise<Map<number, number>> {
    ...
    const rows = await db(Tables.MEMORIAM_PRAYERS)
      .whereIn('deceased_person_id', deceasedIds)
      .groupBy('deceased_person_id')
      .select<{ deceased_person_id: number; count: number }[]>(
        'deceased_person_id',
  ...
  async findTributes(familyId: number, deceasedId: number): Promise<TributeRow[]> {
    return db(`${Tables.MEMORIAM_TRIBUTES} as t`)
      .innerJoin(`${Tables.PERSONS} as p`, 'p.id', 't.author_person_id')
      .where({
  ...
  async findTributePhotosByTributeIds(tributeIds: number[]): Promise<Map<number, string[]>> {
    ...
    const rows = await db(Tables.MEMORIAM_TRIBUTE_PHOTOS)
  ...
  async createTribute(
    ...
    return db.transaction(async (trx) => {
      const [tributeId] = await trx(Tables.MEMORIAM_TRIBUTES).insert({
        family_id: familyId,
  ...
  async updateTribute(
    ...
    await db.transaction(async (trx) => {
      const updated = await trx(Tables.MEMORIAM_TRIBUTES)
        .where({
          id: tributeId,
          family_id: familyId,
          deceased_person_id: deceasedId,
  ...
  async softDeleteTribute(
    ...
    await db(Tables.MEMORIAM_TRIBUTES)
      .where({
        id: tributeId,
        family_id: familyId,
        deceased_person_id: deceasedId,
  ...
  private async syncPhotos(
    ...
    await trx(Tables.MEMORIAM_TRIBUTE_PHOTOS).where({ tribute_id: tributeId }).del();
    if (photoUrls.length === 0) {
      return;
    }
    await trx(Tables.MEMORIAM_TRIBUTE_PHOTOS).insert(
  ...
  async findPrayers(familyId: number, deceasedId: number): Promise<PrayerRow[]> {
    return db(`${Tables.MEMORIAM_PRAYERS} as mp`)
      .innerJoin(`${Tables.PERSONS} as p`, 'p.id', 'mp.author_person_id')
      .where({
  ...
  async findPrayerByAuthor(
    ...
    return db(Tables.MEMORIAM_PRAYERS)
  ...
  async insertPrayer(
    ...
    const [id] = await db(Tables.MEMORIAM_PRAYERS).insert({
      family_id: familyId,
      deceased_person_id: deceasedId,
      author_person_id: authorPersonId,
    });
