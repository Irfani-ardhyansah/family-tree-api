...
describe('tree-subgraph.service', () => {
  ...
  it('showSiblings includes ancestor sibling but not their in-law spouse', () => {
    // Mirror seed: Endang (30) = saudara kakek, Herman (31) = suami Endang (no parents).
    // Herman tidak punya jalur darah ke roots — jangan muncul meski showSpouses.
    const graph: PersonGraphNode[] = [
      node(1, null, null, [2]),
      node(2, null, null, [1], 'female'),
      node(3, 1, 2, [20]),
      node(20, null, null, [3], 'female'),
      node(30, 1, 2, [31]),
      node(31, null, null, [30], 'female'),
      node(4, 3, 20),
      node(5, 4, null),
    ];

    const ids = filterIds(5, 5, graph, {
      lineage: 'both',
      generationsUp: 4,
      showSiblings: true,
      showSpouses: true,
  ...
  it('showSpouses still includes spouse of focus sibling', () => {
    const graph: PersonGraphNode[] = [
      node(10, null, null, [11]),
      node(11, null, null, [10], 'female'),
      node(20, 10, 11, [21]),
      node(21, null, null, [20], 'female'),
      node(22, 10, 11, [23]),
      node(23, null, null, [22], 'female'),
    ];

    const ids = filterIds(20, 20, graph, {
      lineage: 'both',
      generationsUp: 1,
      showSiblings: true,
      showSpouses: true,
