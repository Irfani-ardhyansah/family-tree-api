...
import { isLegalAge, formatBirthDate } from '../../core/auth/auth.mapper';
import {
  attachResolvedMedia,
...
function parseListGenderParam(raw: unknown): 'male' | 'female' | undefined {
  ...
  throw new AppError(
    ...
    'Parameter gender harus male atau female.',
  );
}
...
function validateUpsertInput(input: unknown): UpsertPersonInput {
  ...
  const fatherId = parseOptionalParentId(body.fatherId, 'ayah');
  const motherId = parseOptionalParentId(body.motherId, 'ibu');
...
function parseListQuery(raw: Record<string, unknown>): PersonListQuery {
  ...
  const gender = parseListGenderParam(raw.gender);
  ...
  return { page, limit, view, scope, q, gender };
}
...
export class PersonsService {
  ...
  async list(
    ...
    if (query.gender) {
      filteredRows = filteredRows.filter((row) => row.gender === query.gender);
    }
    if (query.q) {
      filteredRows = filteredRows.filter((row) => matchesPersonWordSearch(row, query.q!));
    }
  ...
  private async assertRelatedPersonsInFamily(
    ...
    for (const { id, role } of parentChecks) {
      ...
      assertParentGender(parent.gender, role);
      assertParentBornBeforeChild(
        input.birthDate,
