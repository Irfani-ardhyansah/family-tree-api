...
export class MediaRepository {
  ...
  async findById(id: string): Promise<MediaRow | undefined> {
    return db(Tables.MEDIA).where({ id }).first<MediaRow>();
  }
  ...
  async findByIds(ids: string[]): Promise<MediaRow[]> {
    ...
    return db(Tables.MEDIA).whereIn('id', ids).select<MediaRow[]>('*');
  }
  ...
  async findByUrls(urls: string[]): Promise<MediaRow[]> {
    ...
    return db(Tables.MEDIA).whereIn('url', urls).select<MediaRow[]>('*');
  }

  async softDelete(id: string): Promise<void> {
    await db(Tables.MEDIA).where({ id }).update({
  ...
  async softDeleteMany(ids: string[]): Promise<void> {
    ...
    await db(Tables.MEDIA).whereIn('id', ids).update({
      status: 'deleted' satisfies MediaStatus,
      deleted_at: db.fn.now(),
      updated_at: db.fn.now(),
    });
