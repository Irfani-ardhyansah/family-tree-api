...
export class PersonImportRepository {
  async insertJob(input: {
    ...
    await db('person_import_jobs').insert({
      id: input.id,
  ...
  async findById(jobId: string): Promise<PersonImportJobRow | undefined> {
    return db('person_import_jobs').where({ id: jobId }).first<PersonImportJobRow>();
  }
  ...
  async updateProgress(
    ...
    await db('person_import_jobs').where({ id: jobId }).update(update);
  }
